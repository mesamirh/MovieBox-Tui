...
| **Windows** | Supported for search and playback. Install `mpv` with winget (see below) and keep it on `PATH`. Inline poster images are still experimental depending on the terminal. |
...
**Windows (recommended):** install [shinchiro’s mpv](https://github.com/shinchiro/mpv-winbuild-cmake) with winget in an **Administrator** terminal, then make sure `mpv` is on your `PATH`:

```powershell
# Run PowerShell or Windows Terminal as Administrator
winget install --id shinchiro.mpv -e

# Confirm mpv is on PATH (open a NEW terminal after install)
where.exe mpv
mpv --version
```

If `where.exe mpv` finds nothing, locate `mpv.exe` and add **that folder** to your user `PATH`:

```powershell
# Example: find mpv.exe under WinGet packages, then add its directory
Get-ChildItem -Path "$env:LOCALAPPDATA\Microsoft\WinGet\Packages" -Filter mpv.exe -Recurse -ErrorAction SilentlyContinue |
  Select-Object -First 1 -ExpandProperty DirectoryName
```

Copy the printed folder path, then:

```powershell
$mpvDir = "PASTE_FOLDER_WITH_mpv.exe_HERE"
[Environment]::SetEnvironmentVariable(
  "Path",
  [Environment]::GetEnvironmentVariable("Path", "User") + ";$mpvDir",
  "User"
)
```

Or add it via **Settings → System → About → Advanced system settings → Environment Variables → Path**.

Restart the terminal (and MovieBox-Tui) after changing `PATH`.

MovieBox-Tui auto-detects `mpv` from `PATH`, and on Windows also checks common install folders such as `C:\Program Files\MPV Player\mpv.exe`.
...
Install `mpv` (see [Requirements](#requirements)), then **restart the terminal** so `PATH` updates are picked up. Player detection runs at startup.

**Windows:** prefer an elevated install:

```powershell
# Administrator PowerShell / Windows Terminal
winget install --id shinchiro.mpv -e
```
...
If that fails, add the folder that contains `mpv.exe` to your user `PATH` and open a new terminal before launching MovieBox-Tui.

On macOS/Linux you can also press <kbd>o</kbd> to pick `IINA` or `VLC` if those are installed.
...
On Windows, playback streams through a local proxy into `mpv` (no full-file download required). If the CDN returns **HTTP 429** (rate limited), wait a few minutes and press play once — do not spam play. You can still use <kbd>d</kbd> to download if streaming stays blocked.
