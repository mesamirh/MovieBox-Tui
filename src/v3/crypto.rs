...
mod tests {
    ...
    fn test_build_canonical_string() {
        let ts = 1000;
        let url = "https://api.example.com/test?b=2&a=1";
        let canonical = build_canonical_string("GET", Some("application/json"), Some("application/json"), url, None, ts);
    ...
    fn test_generate_x_tr_signature() {
        ...
        let sig = generate_x_tr_signature("GET", Some("application/json"), Some("application/json"), url, None, ts);
        assert!(sig.starts_with("1000|2|"));
    }
}
