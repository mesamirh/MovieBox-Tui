...
fn search_hint(view: SearchViewState, width: u16, theme: &Theme) -> Line<'static> {
    let text = match view {
        ...
        SearchViewState::Error if width >= 78 => {
            "[Enter] Retry  [Type] Edit  [Ctrl+L] Library  [Esc] Clear"
        }
        ...
        SearchViewState::Results if width >= 86 => {
            "[Type] Edit  [↑↓] Browse  [Enter] Open  [Ctrl+L] Library"
        }
        ...
        SearchViewState::NoResults if width >= 78 => {
            "[Type] Edit  [Enter] Retry  [Ctrl+L] Library  [Esc] Clear"
        }
        SearchViewState::NoResults if width >= 62 => "[Type] Edit  [Enter] Retry  [Esc] Clear",
        SearchViewState::NoResults => "[Type] Edit  [Esc] Clear",
        SearchViewState::Loading => "",
        SearchViewState::Empty if width >= 54 => "[Type] Search  [Ctrl+L] Library  [?] Help",
        SearchViewState::Empty => "[Ctrl+L] Library  [?] Help",
    };
        ...
        if state.tick_count >= 3 {
            ...
            let footer = if area.width >= 64 {
                Line::from(vec![
                    ...
                    Span::raw("    "),
                    Span::styled("[", theme.text_dim),
                    Span::styled("Ctrl+L", theme.shortcut),
                    Span::styled("] ", theme.text_dim),
            ...
            .map(|scope| {
                ...
                match scope {
                    crate::providers::models::SearchScope::All => {
                        format!("{mark}All providers  (MovieBox + 4KHDHub)")
                    }
